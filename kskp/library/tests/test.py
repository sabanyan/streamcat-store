...
class LibraryTest(unittest.TestCase):
    ...
    def test_get_frame(self):
        ...
        frame = Library.save_frame(root.uuid, 'フレームデータ', Path('kskp/data/library/aaaa.csv'), self.USER_ID1)
        # 作成したフレームを取得する
    ...
    def test_save2_frame(self):
        ...
        # フレームデータを格納するファイルを作成する
        self.save('kskp/data/library/aaaa.csv')
        with open('kskp/data/library/aaaa.csv', mode='rb') as stream:
            # ルートデータストアの直下にフレームを作成する
    ...
    def test_delete_folder_has_child(self):
        ...
        self.save('kskp/data/library/aaaa.csv')
        # ルートデータストアの直下にフレームを作成する
        frame = Library.save_frame(folder.uuid, 'フレームデータ', Path('kskp/data/library/aaaa.csv'), self.USER_ID1)
    ...
    def test_delete_frame_refer_to_file_other_frame_refering(self):
        ...
        root = Library.load_root()
        # フレームデータを格納するファイルを作成する
        self.save('kskp/data/library/foo.csv')
        # ルートデータストアの直下にフレーム1を作成する
        frame1 = Library.save_frame(root.uuid, 'フレームデータ', Path('kskp/data/library/foo.csv'), self.USER_ID1)
        # ルートデータストアの直下にフレーム2を作成する
        frame2 = Library.save_frame(root.uuid, 'フレームデータ', Path('kskp/data/library/foo.csv'), self.USER_ID1)
        # フレーム1を削除する
        Library.delete_frame(frame1.uuid)
        # フレーム1,2に対応するCSVファイルが存在することを検証する
        self.assertTrue(os.path.isfile('kskp/data/library/foo.csv'))
        # フレーム2を削除する
        Library.delete_frame(frame2.uuid)
        # フレーム1,2に対応するCSVファイルが存在しないことを検証する
        self.assertFalse(os.path.isfile('kskp/data/library/foo.csv'))
    ...
    def test_update_frame_refer_to_file_other_frame_refering(self):
        ...
        root = Library.load_root()
        # フレームデータを格納するファイルを作成する
        self.save('kskp/data/library/abc.csv')
        # ルートデータストアの直下にフレーム1を作成する
        frame1 = Library.save_frame(root.uuid, 'フレームデータ', Path('kskp/data/library/abc.csv'), self.USER_ID1)
        # ルートデータストアの直下にフレーム2を作成する
        frame2 = Library.save_frame(root.uuid, 'フレームデータ', Path('kskp/data/library/abc.csv'), self.USER_ID1)
        # フレーム1のラベル名を変更する
        Library.update_frame_data(frame1.uuid, '新しいフレームデータ1', self.USER_ID2)
        # フレーム1のラベル名の変更に従って、CSVファイル名が変更されていることを検証する
        self.assertEqual(frame1.path, 'kskp/data/library/新しいフレームデータ1')
        self.assertEqual(frame2.path, 'kskp/data/library/新しいフレームデータ1')
        self.assertTrue(os.path.isfile('kskp/data/library/新しいフレームデータ1'))
        self.assertFalse(os.path.isfile('kskp/data/library/abc.csv'))
        # フレーム1を削除する
        Library.delete_frame(frame1.uuid)
        # フレーム2を削除する
        Library.delete_frame(frame2.uuid)
    ...
    def test_frame_file_collision(self):
        ...
        root = Library.load_root()
        # フレームデータを格納するファイルを作成する
        self.save('kskp/data/library/bar.csv')
        with open('kskp/data/library/bar.csv', mode='rb') as stream:
            # ルートデータストアの直下にフレーム1を作成する
        ...
        self.assertEqual(frame1.path, 'kskp/data/library/フレームデータ')
        self.assertEqual(frame2.path, 'kskp/data/library/フレームデータ_1')
        self.assertEqual(frame3.path, 'kskp/data/library/フレームデータ_2')
        # フレーム1を削除する
        Library.delete_frame(frame1.uuid)
        # フレーム2を削除する
        Library.delete_frame(frame2.uuid)
