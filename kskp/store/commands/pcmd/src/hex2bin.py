...
例
●入力      ●参照                        ●指定             ●出力
id,HEX     H1,H2,H3,H4,H5,H6,H7,H8     f=H1,H8          id,HEX,H1,H8
1,00                                   Hex=HEX    =>>   1,00,0,0
2,0f                                                    2,0f,0,1
3,ff                                                    3,ff,1,1
...
●パフォーマンス
    ...
    １．欲しい値の取得法の見直し
      欲しい値のインデックスを、イテレーションしている。
      →複数indexに対応した値が効率的に取得できれば、処理速度が上がる
    
    2.対象列の取得
      参照データの列数x指定列数分、イテレーションしている。
      →インデックス取得にすれば、イテレーション数を減らして高速化できる
       ただ、全体の処理時間削減としては、それほど寄与しないと推測（処理全体で１回しかやっていないから）
    ...
    for line in gen_reader_i:
        ...
        # 変換処理
            ...
            if is_hex(val):
                ...
                bit_val = ' ' * len(val) * 4 # 変換できない値はNullとして空白を出力する

            #2019.09.15 までの処理(高速化のため上に変更した)
            ...
            #2019.09.17修正
            for idx in sel_dgts:
                sel_vals.append(bit_val[idx])

            # 2019.09.17 修正前（修正目的：高速化）
