...
async def init_admin_users():
    async with UnAuthzFinder() as ufinder:
        ...
        # システム管理者とユーザ管理者を作成する
        sys_admin_user = await ufinder.load_sys_admin_user()
        usr_admin_user = await ufinder.load_usr_admin_user(activate_if_inactive=True)

        sys_finder = await ufinder.create_authz_finder(sys_admin_user)
        sys_admin_role = sys_finder.role.load_sys_admin_role()
        ...
        usr_finder.role.load_usr_admin_role()

        # everyoneロールにユーザ管理者を所有者として参加させる
        # (unauthz_finderからロールを新規追加された場合、作成者はロールに参加されない)
        # (ユーザ管理者ロールを作成した後に処理すること)
        everyone_role = usr_finder.role.load_everyone_role()
        # edit_lock_roleロールにユーザ管理者を所有者として参加させる
        edit_lock_role = usr_finder.role.load_edit_lock_role()
        ...
        # システムフォルダを作成する
        usr_finder.data.load_cache_folder()
        usr_finder.data.load_activity_folder()
        usr_finder.data.load_trash_folder()

        # ライブラリにある全てのスケジュールをスケジューラに登録する
        from ..scheduler import schedule_manager
        schedule_manager.load_from_library(usr_finder.data)
